...
2. To see $M_g$ retracts on to $C'$, notice that $M_g$ is the a $g-1$ genus surface minus a disk glued to a torus minus a disk, the map from $M_g \to M_1$ quotienting the $g-1$ genus surface minus a disk to a point, composition of this quotient map with a retraction $M_1\to C'$ forms the final retraction. For the planar presentation, $M_g$ has a cell structure of $4 g$ 1-cells (labelled by $a_1, b_1, \cdots, a_g, b_g$) and 1 2-cell. First map $M_g \to M_1$ collapses the $(4g-4)$ sides to a point. Then we get a square with boundary $a_1b_1a_1^{-1}b_1^{-1}$. Mapping $b_1$ to a point gives the retract to the circle represented by $a_1$, and the composition is the desired retraction.
...
First, consider $X=S^1 \vee S^1$. Denote the two circles by $a$ and $b$. Note that $T_f$ has a CW structure with one 0-cell $x_0$, three 1-cells $a, b, c$ attached to $x_0$ ($a,b$ are the two $S^1$ in $X$ and $c$ is the quotient $`\{x_0\}\times[0,1]/0\sim1`$), and two 2-cells. The boundaries of these 2-cells are attached along the loops $`a c f_*(a)^{-1} c^{-1}`$ and $`b c f_*(b)^{-1} c^{-1}`$ respectively. Thus, we have the following presentation for $\pi_1(T_f)$:
...
The fundamental group of a surface of genus $g$ with $b$ boundary components is a free group on $2g+b-1$ generators. For $Y$, we have $g=1$ and $b=2$, so its fundamental group is the free group on $2(1)+2-1=3$ generators, i.e., $\pi_1(Y) \cong \mathbb{Z} * \mathbb{Z} * \mathbb{Z}$, and each boundary circle of the removed disk is a generator in $\mathbb{Z} \hookrightarrow \mathbb{Z} * \mathbb{Z} * \mathbb{Z}$. The surface of infinite genus consists of infinitely many copies of $Y$, attached on the boundary circles. By van Kampen's theorem, after gluing two copies of $Y$ together, the generator representing the glued circle from each group are identified, the fundamental group becomes $\mathbb{Z} * \mathbb{Z} * \mathbb{Z} * \mathbb{Z} * \mathbb{Z} * \mathbb{Z}$. Keep doing the gluing, we know the fundamental group is free on an infinite number of generators.
