...
Since $Y$ has a deformation retract on a circle with a diameter, we know $\pi_1(Y) \cong \pi_1\left(S^1 \vee S^1\right) \cong \mathbb{Z} * \mathbb{Z} * \mathbb{Z}$, and each boundary circle of the removed disk is a generator in $\mathbb{Z} \hookrightarrow \mathbb{Z} * \mathbb{Z} * \mathbb{Z}$. The surface of infinite genus consists of infinitely many copies of $Y$, attached on the boundary circles. By van Kampen's theorem, after gluing two copies of $Y$ together, the fundamental group becomes $\mathbb{Z} * \mathbb{Z} * \mathbb{Z} * \mathbb{Z} * \mathbb{Z} * \mathbb{Z}$. Keep doing the gluing, we know the fundamental group is free on an infinite number of generators.
