...
Now, we need to determine the number of elements in the double quotient $H_{m_1}⧹S_n / H_{m_2}$. This double quotient can be interpreted as the set of orbits of the action of $H_{m_1}$ on the set of $m_2$-element subsets of $`\{1, \ldots, n\}`$. Two $m_2$-element subsets are in the same orbit under this action if one can be transformed into the other by a permutation that only rearranges the first $m_1$ elements and the last $n - m_1$ elements separately. The invariant that characterizes these orbits is the size of the intersection of an $m_2$-element subset with the specific subset $`\{1, \ldots, m_1\}`$ stabilized by $H_{m_1}$. If we denote this intersection size by $k$, then $k$ can take values from $\max(0, m_1 + m_2 - n)$ to $\min(m_1, m_2)$. The upper bound is clear since the intersection cannot be larger than either subset. The lower bound follows from the inclusion-exclusion principle: the union of the two subsets has size $m_1 + m_2 - k$ and is contained in a set of size $n$, so $m_1 + m_2 - k \le n$. This implies $k \ge m_1 + m_2 - n$. Since $k \ge 0$, the lower bound is $\max(0, m_1 + m_2 - n)$. Therefore, the number of distinct orbits, and hence the number of elements in the double quotient $H_{m_1}⧹S_n / H_{m_2}$, is the count of integers from $\max(0, m_1 + m_2 - n)$ to $\min(m_1, m_2)$, inclusive. This count is given by $\min(m_1, m_2) - \max(0, m_1 + m_2 - n) + 1$.
