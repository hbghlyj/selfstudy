...
Now, we need to determine the number of elements in the double quotient $H_{m_1}⧹S_n / H_{m_2}$. This double quotient can be interpreted as the set of orbits of the action of $H_{m_1}$ on the set of $m_2$-element subsets of $`\{1, \ldots, n\}`$. Two $m_2$-element subsets are in the same orbit under this action if one can be transformed into the other by a permutation that only rearranges the first $m_1$ elements and the last $n - m_1$ elements separately. What remains invariant under this action is the size of the intersection of the two subsets.
