...
There is a natural bijection between $H_1⧹G/H_2$ and $H_2⧹G/H_1$ via the map $H_1 g H_2 \mapsto H_2 g^{-1} H_1$. Therefore, sets (1) and (2) are bijective to each other.
...
Now, we need to determine the number of elements in the double quotient $H_{m_1}⧹S_n / H_{m_2}$. Suppose $m_1\le m_2$. This double quotient can be interpreted as the set of orbits of the action of $H_{m_1}$ on the set of $m_2$-element subsets of $`\{1, \ldots, n\}`$. Two $m_2$-element subsets are in the same orbit under this action if one can be transformed into the other by a permutation that only rearranges the first $m_1$ elements and the last $m_2 - m_1$ elements separately.
