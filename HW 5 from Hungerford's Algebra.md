...
Now, we need to determine the number of elements in the double quotient $H_{m_1}⧹S_n / H_{m_2}$. This double quotient can be interpreted as the set of orbits of the action of $H_{m_1}$ on the set of $m_2$-element subsets of $`\{1, \ldots, n\}`$. Two $m_2$-element subsets are in the same orbit under this action if one can be transformed into the other by a permutation that only rearranges the first $m_1$ elements and the last $n - m_1$ elements separately. The invariant that characterizes these orbits is the size of the intersection of an $m_2$-element subset with the specific subset $`\{1, \ldots, m_1\}`$ stabilized by $H_{m_1}$. If we denote this intersection size by $k$, then $k$ can take values from $\max(0, m_1 + m_2 - n)$ to $\min(m_1, m_2)$. The lower bound arises because the intersection cannot be negative and must account for the total number of elements, while the upper bound is limited by the smaller of the two subset sizes. Therefore, the number of distinct orbits, and hence the number of elements in the double quotient $H_{m_1}⧹S_n / H_{m_2}$, is the count of integers from $\max(0, m_1 + m_2 - n)$ to $\min(m_1, m_2)$, inclusive. This count is given by $\min(m_1, m_2) - \max(0, m_1 + m_2 - n) + 1$.
