...
Let $W$ be a subrepresentation of $V^*$. We want to show that $W$ is either $\{0\}$ or $V^*$.

If $W = \{0\}$, we are done. So suppose $W$ is nontrivial. Then there exists a nonzero functional $f \in W$. Since $f$ is nonzero, there exists some $v \in V$ such that $f(v) \neq 0$.

Now, consider the orbit of $f$ under the action of $G$:

$O_f = \{g \cdot f \mid g \in G\}$

This orbit generates a subrepresentation of $V^*$, which we denote by $U$. Since $W$ is a subrepresentation and contains $f$, it must also contain the entire orbit $O_f$, and hence $U \subseteq W$.

Next, we will show that $U$ is actually the whole space $V^*$. To do this, we will use the fact that $V$ is irreducible.

Since $V$ is irreducible, for any nonzero vector $v \in V$, the orbit of $v$ under the action of $G$ spans the entire space $V$. This means that for any $w \in V$, there exists a linear combination of elements in the orbit of $v$ that equals $w$.

Now, consider any functional $`h \in V^*`$. We can express $h$ in terms of the basis formed by the orbit of $f$. Since $f(v) \neq 0$, we can use the elements of the orbit $O_f$ to construct any functional in $`V^*`$. This implies that the subrepresentation $U$ generated by the orbit of $f$ is actually equal to $`V^*`$.
