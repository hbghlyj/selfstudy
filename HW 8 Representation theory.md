...
Let $`\lambda`$ be an eigenvalue of $`\phi`$, and let $`E_{\lambda} = \{ v \in V : \phi(v) = \lambda v \}`$ be the corresponding eigenspace. Since $`\phi`$ is linear, $`E_{\lambda}`$ is a subspace of $`V`$.
