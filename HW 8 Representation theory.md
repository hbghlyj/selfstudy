...
# 2
Let $V$ be a finite-dimensional vector space, $\operatorname{dim}(V)=n$. For $k \geq 0$, let $G(V, k)$ be the set of all the $k$-dimensional subspaces of $V$ (it is called the Grassmannian of $V$ ). Show that the correspondence
...
$V^* \otimes W \rightarrow \operatorname{Hom}_K(V, W): \phi \otimes w \mapsto \phi(-) w$
...
# 6
Let $G$ be an abelian group and $V$ be a finite-dimensional irreducible representation of $G$ over $\mathbb{C}$. Show that $\operatorname{dim}(V)=1$. (This is closely related to the previous problem.)
