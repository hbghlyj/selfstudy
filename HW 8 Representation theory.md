...
By definition, $V$ is an irreducible representation of $G$ if it has no nontrivial subrepresentations. This means that the only subrepresentations of $V$ are the zero representation and $V$ itself.

Since $G$ is abelian, take any two distinct elements $g_1, g_2 \in G$. Then, $g_1 g_2 = g_2 g_1$. By the previous problem, there exists a vector $v \in V$ that is an eigenvector for both $g_1$ and $g_2$ simultaneously. Continuing this process for all pairs of elements in $G$, we find that $v$ is an eigenvector for every element of $G$. So the subrepresentation generated by $v$ is the entire representation $V$.
