...
Since $G$ is abelian, all operators $\rho(g)$ for $g \in G$ commute. By induction using the result of the previous problem, there exists a non-zero vector $v \in V$ that is a simultaneous eigenvector for all $g \in G$.

The one-dimensional subspace $W = \mathrm{span}(v)$ is a subrepresentation of $V$, because for any $g \in G$, $\rho(g)(v) = \lambda_g v \in W$.
