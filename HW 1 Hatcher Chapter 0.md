...
1. Suppose $X$ is contractible, then there is a point $x_0$, and maps $`h: X \to\{x_0\}, g:\{x_0\} \to X`$ s.t. $`g \circ h \simeq \mathrm{id}|_X`$ and $`h \circ g \simeq \mathrm{id}|_{\{x_0\}}`$. We denote the homotopy as $F: X \times I \to X$ where $`F|_{X \times\{0\}}=\mathrm{id}`$ and $`\left.F\right|_{X \times\{1\}}=g \circ h`$. For any $f: X \to Y$ where $Y$ is an arbitrary space, let $`y_0=f(g(x_0))`$, and let $G:=f \circ F$. Thus $G: X \times I \to Y$ is continuous since it is the composition of two continuous maps. $`G|_{X \times\{0\}}=f \circ \mathrm{id}=f`$ and $`G|_{X \times\{1\}}=f \circ g \circ h`$. But $f \circ g \circ h(X)=y_0$. Therefore $f: X \to Y$ is nullhomotopic.
