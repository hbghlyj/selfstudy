...
Base Case
In this case, we have a 1 -simplex (i.e. an edge) homeomorphic to a compact interval $[a, b]$. Barycentric subdivision gives us two intervals $`I_1\left[a, \frac{a+b}{2}\right], I_2\left[\frac{a+b}{2}, b\right]`$. These clearly have distinct vertices (one has the vertex $a$, the other has $b$ ).
Induction Step
Our induction hypothesis is that if $X$ is a $\Delta$-complex then for all $m<n$, the subcomplex generated by all of the $m$-simplices in $X$ forms a simplicial complex. For each $n$-simplex $`X^n:=\left[x_0, \ldots, x_n\right]`$ with barycenter $b$ in $X$, we have $n$ subsimplices $`X_i^n=\left[b, x_0, \ldots, \hat{x}_i, \ldots, x_n\right]`$. Each of these subsimplices has a boundary

$`\partial X_i^n=\sum_{j<i}(-1)^j\left[b, x_0, \ldots, \hat{x}_j, \ldots, \hat{x}_i, \ldots, x_n\right]+\sum_{j>i}(-1)^{j-1}\left[b, x_0, \ldots, \hat{x}_i, \ldots, \hat{x}_j, \ldots, x_n\right]`$

The $n-1$ simplices that comprise this boundary satisfy the induction hypothesis, so that $`\left[b, x_0, \ldots, \hat{x}_i, \ldots, \hat{x}_j, \ldots, x_n\right] \neq \left[b, x_0, \ldots, \hat{x}_{i'}, \ldots, \hat{x}_{j'}, \ldots, x_n\right]`$ unless $i=i', j=j'$. Note that all of the vertices in $X^n$ are contained in $\partial X_i^n, \partial X_j^n$ as long as $i \neq j$. Since each $X_i^n$ of the barycentric subdivision has a different vertex $x_i$ removed, this shows that $X_i^n=X_j^n \Longleftrightarrow i=j$ which means that all the subsimplices have distinct vertices. Hence $B\left(X^n\right)$ is made up $n$-simplices that all have distinct vertices.
...
By hypothesis, $X^1$ comprises of 1 -simplices $X_i^1$. Now suppose that $X_i^1, X_j^1$ are 1 -simplices such that $\partial X_i^1=\partial X_j^1=\{a, b\}$ for some $a, b \in X^0$. Then under barycentric subdivsion, we introduce two points $`c_k \in X_k^1, k \in\{i, j\}`$ such that

$`B\left(X_j^1\right) \cong\left[a, c_i\right] \cup\left[c_i, b\right], k \in\{i, j\}`$. Since $c_i \neq c_j \neq a \neq b$ this implies that $B\left(X_i^1\right), \forall i$ has distinct vertices. Since this holds for arbitrary $X_i^1$, this means that $B\left(X^1\right)$ is comprised of simplices with different vertices
