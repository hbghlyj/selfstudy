...
3. If $G$ is cyclic of prime order $p$, then its only subgroups are $`\{e\}`$ and $G$ itself, so $s(G) = 2$. Conversely, if $s(G) = 2$, then $G$ has no nontrivial subgroups. Any nontrivial element must generate $G$, so $G$ is cyclic of prime order.
