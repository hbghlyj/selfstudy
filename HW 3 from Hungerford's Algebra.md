...
- $f(g(b))=f(x^2)=(ab^{-1})^2=a^2(b^{-1})^2=(b^{-1})^2=b^{-2}=b$
...
5. **Case 1: $|G|$ is not a prime power** Let $H$ and $K$ be our two nontrivial proper subgroups. If $|G|$ has at least two distinct prime divisors $p,q$, then by Cauchy's theorem, $H = \langle x \rangle$ and $K = \langle y \rangle$ where $|x| = p$ and $|y| = q$. Since their orders must be coprime, their intersection must be trivial, so $G = \langle H,K \rangle$. In particular, $H$ and $K$ are unique subgroups of particular orders, so they both must be normal, hence $G \cong \mathbb{Z}/p\mathbb{Z} \times \mathbb{Z}/q\mathbb{Z}$ which is cyclic by the Chinese remainder theorem.

   **Case 2: $G$ is a $p$-group** Suppose instead that $G$ is a $p$-group. We know that if $|G| = p^n$, then there exists a subgroup order $p^k$ for each $k=0,\ldots,n$. Let $|H| = p$ and $|K| = p^2$. It is the case that either $K \cong \mathbb{Z}/p^2\mathbb{Z}$ which we showed has exactly three subgroups, or $\mathbb{Z}/p\mathbb{Z} \times \mathbb{Z}/p\mathbb{Z}$ (which is called the elementary abelian group of order $p^2$); it is another simple exercise to show that this group has $p+3 \geq 5$ subgroups (The group $\mathbb{Z}/p\mathbb{Z} \times \mathbb{Z}/p\mathbb{Z}$ has $p+3$ subgroups because, by Lagrange's theorem, possible subgroup orders are $1,p,$ and $p^{2}$. There are exactly two subgroups of order $1$ and $p^{2}$ (the trivial subgroup and the group itself). To count the subgroups of order $p$, you can count the number of elements of order $p$ in the group ($p^{2}-1$) and divide by the number of elements in each order-$p$ subgroup ($p-1$), resulting in $(p^{2}-1)/(p-1)=p+1$ subgroups of order $p$. Summing these up gives $1+(p+1)+1=p+3$ total subgroups), so $K$ must be cyclic. Either way, we have that $H \leq K$. Therefore, we can use our argument for $s(G) = 3$ to conclude that $G$ must be cyclic.
