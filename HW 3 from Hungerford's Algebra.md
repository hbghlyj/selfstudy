...
5. Let $H$ and $K$ be our two nontrivial proper subgroups. If $|G|$ has at least two distinct prime divisors $p,q$, then by Cauchy's theorem, $H = \langle x \rangle$ and $K = \langle y \rangle$ where $|x| = p$ and $|y| = q$. Since their orders must be coprime, their intersection must be trivial, so $G = \langle H,K \rangle$. In particular, $H$ and $K$ are unique subgroups of particular orders, so they both must be normal, hence $G \cong \mathbb{Z}/p\mathbb{Z} \times \mathbb{Z}/q\mathbb{Z}$ which is cyclic by the Chinese remainder theorem.
