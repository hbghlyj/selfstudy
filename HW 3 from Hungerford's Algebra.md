...
Let $z$ be a non-identity element in the center of $G * H$. Then $z$ must contain at least one letter from $G$ and one letter from $H$. Without loss of generality, we can assume that the reduced word $z$ starts with a letter from $G$. Then $zh$ starts with a letter from $G$, while the reduced word $hz$ starts with a letter from $H$. Thus $zh\neq hz$, a contradiction. Therefore, the center of $G * H$ is trivial.
