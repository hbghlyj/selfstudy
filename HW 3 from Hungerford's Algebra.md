...
Let $G = A * B$. By the universal property of free products, there exists a natural surjective homomorphism $\phi: G \to B$ defined by $\phi(a) = e_B$ for all $a \in A$ and $\phi(b) = b$ for all $b \in B$, where $e_B$ is the identity element of $B$. The kernel of this homomorphism is precisely the normal subgroup $N$, since it consists of all elements of $G$ that map to the identity in $B$. By the first isomorphism theorem, we have $(A * B) / N \cong B$.
