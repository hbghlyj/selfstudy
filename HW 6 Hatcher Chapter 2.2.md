...
The homology group $H_n(X^n)$ can be computed using the long exact sequence of the pair $(X^n, X^{n-1})$:

$`\cdots \rightarrow H_n(X^{n-1}) \rightarrow H_n(X^n) \rightarrow H_n(X^n, X^{n-1}) \xrightarrow{d_n} H_{n-1}(X^{n-1}) \rightarrow \cdots`$

Since $H_n(X^{n-1}) = 0$ for $n > 0$ (as $X^{n-1}$ has no $n$-cells), the map from $H_n(X^{n-1})$ to $H_n(X^n)$ is the zero map. Therefore, we have an isomorphism:

$`H_n(X^n) \cong \ker(d_n: H_n(X^n, X^{n-1}) \rightarrow H_{n-1}(X^{n-1}, X^{n-2}))`$

The group $H_n(X^n, X^{n-1})$ is free abelian, generated by the $n$-cells of $X$. The boundary map $d_n$ is a homomorphism between free abelian groups. The kernel of a homomorphism between free abelian groups is also free abelian. Therefore, $H_n(X^n)$, being isomorphic to the kernel of $d_n$, is free abelian.
