...
By definition, the Lie bracket $[Y, X]$ at a point $p \in M$ is given by
$`[Y, X]_p = \lim_{t \to 0} \frac{(\eta_{-t})_* X_{\eta_t(p)} - X_p}{t}.`$

Using the definition of the pushforward on a vector field generated by a flow, we have for any $t$:

$`(\eta_{-t})_* X_{\eta_t(p)} = \frac{d}{du}\big|_{u=0} (\eta_{-t} \circ \theta_u \circ \eta_t)(p)`$.

Since the flows commute, we can rewrite this as:

$`(\eta_{-t})_* X_{\eta_t(p)} = \frac{d}{du}\big|_{u=0} (\eta_{-t} \circ \eta_t \circ \theta_u)(p) = \frac{d}{du}\big|_{u=0} \theta_u(p) = X_p.`$
...
$[X, Y] = 0$ implies that $d\theta_t(Y_{\eta_s(p)}) = Y_{F(t, s, p)}$.
...
$[X, Y] = 0$ implies that $d\eta_s(X_{\theta_t(p)}) = X_{G(t, s, p)}$.

Both $F$ and $G$ satisfy the same system of partial differential equations with the same initial condition $F(0, 0, p) = G(0, 0, p) = p$. By the uniqueness theorem for systems of partial differential equations we conclude that $F(t, s, p) = G(t, s, p)$ for all $t, s \in \mathbb{R}$ and $p \in M$.
