...
The 1-chain group $C_1(X)$ is generated by $\{a_i, b_i, c_i \mid i=1, \ldots, n\} \cup \{d\}$. The boundary maps are given by $\partial_2(f_i) = b_i + c_i - d$, $\partial_2(g_i) = a_i + b_i - d$, and $\partial_2(h_i) = a_i + c_i - d$.

From these, we can derive that $2a_i = 2b_i = 2c_i = d$ for each $i$. Thus, the image of $\partial_2$ is generated by $\{2a_i, 2b_i, 2c_i \mid i=1, \ldots, n\}$. The kernel of $\partial_1$ consists of all 1-chains that sum to zero at the single vertex $v$, which means any combination of edges that results in an even number of each edge type.
