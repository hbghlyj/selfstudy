...
Let $a_i, b_i, c_i, d_i, e_i, f_i$ be the edges of each tetrahedron $T_i$. By the identifications, we have $a_i = a_{i+1}$, $d_i = b_{i+1}$, and $f_i = c_{i+1}$.

The 0-chain group $C_0$ is generated by the vertices, which are all identified to a single vertex $v$. Thus, $H_0 = \mathbb{Z}$.

Let $\alpha_i, \beta_i, \gamma_i, \delta_i$ be the faces of each tetrahedron $T_i$. The identifications give $\beta_i = \alpha_{i+1}$ and $\delta_i = \gamma_{i+1}$. The 1-chain group $C_1$ is generated by $\{a_0, b_0, c_0\}$. The boundary maps are given by:
