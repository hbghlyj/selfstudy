...
The 1-chain group $C_1(X)$ is generated by $`\{a_i, b_i, c_i \mid i=1, \ldots, n\} \cup \{d\}`$. The boundary maps are given by $\partial_2(f_i) = a_i + b_i + c_i - d$, $\partial_2(g_i) = a_i + b_i + d - c_i$, and $\partial_2(h_i) = a_i + c_i + d - b_i$.
