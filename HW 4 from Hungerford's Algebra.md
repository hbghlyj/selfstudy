...
Let $G / C(G) = \langle gC(G) \rangle$ for some $g \in G$. Then for any $x, y \in G$, we can write

$`x = g^m c_1`$ and $`y = g^n c_2`$ for some integers $m, n$ and $c_1, c_2 \in C(G)$.

Now, $xy = (g^m c_1)(g^n c_2) = g^m g^n c_1 c_2 = g^{m+n} c_1 c_2$. The second equality holds because $c_1 \in C(G)$ commutes with $g^n$.

Similarly, $yx = (g^n c_2)(g^m c_1) = g^n g^m c_2 c_1 = g^{n+m} c_2 c_1$, as $c_2 \in C(G)$ commutes with $g^m$.

Since $c_1$ and $c_2$ commute (as they are in $C(G)$), and integer addition is commutative ($m+n=n+m$), it follows that $xy = yx$. Thus, $G$ is abelian.
