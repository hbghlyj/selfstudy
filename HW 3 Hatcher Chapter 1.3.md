...
Now we prove that $`\widetilde{X}`$ is compact. Each $x \in X$ has an evenly covered open neighborhood $U_x$, so $`\{U_x\}_{x \in X}`$ is an open cover of $X$. Since $X$ is compact, this cover has a finite subcover $`\{U_j\}`$. As a compact Hausdorff space, $X$ is normal, so there is a finite open cover $`\{V_j\}`$ with $`\overline{V_j} \subseteq U_j`$. Each $`\overline{V_j}`$ is a closed subset of the compact space $X$, and is therefore compact. For each $j$, $`p^{-1}(\overline{V_j})`$ is compact. This is because $`p^{-1}(U_j)`$ is a disjoint union of a finite number of sheets, each homeomorphic to $U_j$. The preimage $`p^{-1}(\overline{V_j})`$ is the union of the preimages of $`\overline{V_j}`$ in each sheet. Each of these is compact (being homeomorphic to $`\overline{V_j}`$), and there are finitely many, so their union $`p^{-1}(\overline{V_j})`$ is compact. Since $`\{\overline{V_j}\}`$ covers $X$, the collection $`\{p^{-1}(\overline{V_j})\}`$ covers $`\widetilde{X}`$. Thus, $`\widetilde{X} = \bigcup_j p^{-1}(\overline{V_j})`$ is a finite union of compact sets, which implies $`\widetilde{X}`$ is compact.
