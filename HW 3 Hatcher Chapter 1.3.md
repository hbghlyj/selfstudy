...
Our subgroup is the unique normal subgroup of index 8 containing $a^2$, $b^2$, and $(ab)^4$, so we seek a normal, 8-sheeted cover $p\colon \tilde X \to S^1 \vee S^1$ such that the image of $p_*$ contains those elements; it will necessarily be unique.
