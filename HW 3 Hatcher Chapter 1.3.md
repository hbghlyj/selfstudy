...
For $n=3$ and $g \geq 3$, we can construct such a covering space explicitly. Consider the standard embedding of $M_g$ in $\mathbb{R}^3$ as a surface with $g$ handles. The universal abelian cover of $M_g$ corresponds to the kernel of the map $\pi_1(M_g) \to H_1(M_g) \to \mathbb{Z}^3$. The covering space can be visualized as an infinite periodic tiling of $\mathbb{R}^3$ by copies of $M_g$, where the deck transformations are translations by vectors in a lattice generated by three independent vectors in $\mathbb{R}^3$.
