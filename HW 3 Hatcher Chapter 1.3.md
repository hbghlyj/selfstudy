...
For $n=3$ and $g \geq 3$, we can construct the covering space explicitly as a subspace of $\mathbb{R}^3$. The boundary of $`\{(x,y,z)\in\mathbb{R}^3\mid\text{dist}(x,\mathbb{Z})+\text{dist}(y,\mathbb{Z})+\text{dist}(z,\mathbb{Z})=0.1\}`$ is a connected, noncompact, periodic surface in $\mathbb R^3$.
