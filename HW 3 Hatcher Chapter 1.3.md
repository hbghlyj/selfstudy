...
Now we prove that $X$ is compact. To do so, we first show that $X$ is covered by finitely many compact sets. Each $x \in X$ has an evenly covered open neighborhood $U_x$, meaning $\left\{U_x\right\}_{x \in X}$ form an open cover of $X$. Since $X$ is compact, this open cover has a finite sub-cover $\left\{U_j\right\}$ of $X$. Since compact Hausdorff spaces are also normal, $X$ is normal; hence, there is a refinement $\left\{V_j\right\}$ of the covering $\left\{U_j\right\}$ such that each $\overline{V_j} \subseteq U_j$. That is, $\left\{\overline{V_j}\right\}$ is a finite cover of closed sets of $X$. Furthermore, since each $\overline{V_j}$ is closed and bounded, $\left\{\overline{V_j}\right\}$ is a finite cover of compact sets of $X$. Since $p^{-1}$ is homeomorphic on each $U_j$ (and $\overline{V_j} \subseteq U_j$ ), $p^{-1}$ is homeomorphic (and continuous in particular) on each compact $\overline{V_j}$. Thus, $p^{-1}\left(\overline{V_j}\right)$ is compact for each $j$ (since the continuous image of compact sets is compact). For any $x \in \widetilde{X}, p(x) \in \overline{V_j}$ for some $j$ (definition of a cover) and thus $x \in p^{-1}\left(\overline{V_j}\right)$. Since $x \in \widetilde{X}$ was arbitrary, $\left\{p^{-1}\left(\overline{V_j}\right)\right\}$ is a finite cover of compact sets of $\widetilde{X}$.

Using this cover of compact sets, we now show that $\widetilde{X}$ is compact. Let $\left\{U_\alpha\right\}$ be an open cover of $\widetilde{X}$. Then $\left\{U_\alpha\right\}$ is an open cover for each compact $p^{-1}\left(\overline{V_j}\right)$, so there is a finite sub-cover $\left\{U_{\alpha_j}\right\}$ for each $p^{-1}\left(\overline{V_j}\right)$ - so since there are finitely many $j$, the collection of all $\left\{U_{\alpha_j}\right\}$ is a finite sub-cover of $\widetilde{X}$. Hence, $\widetilde{X}$ is compact. Therefore, $\widetilde{X}$ is compact Hausdorff.
...
- its “south pole” $\rightarrow$ the vertex $n+1$.
