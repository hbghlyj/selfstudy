...
* **($\Rightarrow$)** Suppose $\tilde{X}$ is compact Hausdorff. Since $p^{-1}(\{x\}) \neq \emptyset$ for all $x \in X, p$ is surjective. Since the continuous image of compact sets is compact, and $\widetilde{X}$ is compact, we have that $p(\widetilde{X})=X$ is compact as well. Thus, $X$ is compact Hausdorff.
