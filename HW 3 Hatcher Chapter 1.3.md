...
Now we prove that $\widetilde{X}$ is compact. To do so, we first show that $\widetilde{X}$ is covered by finitely many compact sets. Each $x \in X$ has an evenly covered open neighborhood $U_x$, meaning $`\{U_x\}_{x \in X}`$ form an open cover of $X$. Since $X$ is compact, this open cover has a finite sub-cover $\{U_j\right\}$ of $X$. Since compact Hausdorff spaces are also normal, $X$ is normal; hence, there is a refinement $`\{V_j\}`$ of the covering $`\{U_j\}`$ such that each $\overline{V_j} \subseteq U_j$. That is, $`\{\overline{V_j}\}`$ is a finite cover of closed sets of $X$. Furthermore, since each $\overline{V_j}$ is closed and bounded, $`\{\overline{V_j}\}`$ is a finite cover of compact sets of $X$. Since each $U_j$ is evenly covered, $p^{-1}(U_j)$ is a disjoint union of open sets (sheets), each homeomorphic to $U_j$ via the restriction of $p$. Since $\overline{V_j} \subseteq U_j$, each component of $p^{-1}(\overline{V_j})$ is the closure of a component of $p^{-1}(V_j)$ within a sheet of $p^{-1}(U_j)$. The restriction of $p$ to each component of $p^{-1}(\overline{V_j})$ is a homeomorphism onto $\overline{V_j}$, and therefore each component of $p^{-1}(\overline{V_j})$ is compact (as the homeomorphic image of a compact set). For any $x \in \widetilde{X}, p(x) \in \overline{V_j}$ for some $j$ (definition of a cover) and thus $x \in p^{-1}(\overline{V_j})$. Since $x \in \widetilde{X}$ was arbitrary, $`\{p^{-1}(\overline{V_j})\}`$ is a finite cover of compact sets of $\widetilde{X}$.
