...
For $n=3$ and $g \geq 3$, we can construct the covering space explicitly as a subspace of $\mathbb{R}^3$. The boundary of $`\{p\in\mathbb{R}^3\mid\text{dist}(p,\mathbb{Z}^3)=0.1\}`$ is a connected, noncompact, periodic surface in $\mathbb R^3$.

Work in the quotient $T^3=\mathbb{R}^3/\langle(g-2,0,0),(0,1,0),(0,0,1)\rangle$. The $\mathbb Z^3$–periodic “hollow cube” has boundary made of the six outer faces at $\{x,y,z\}\equiv 0\pmod 1$ and the six inner faces at $\{x,y,z\}\equiv \tfrac12 \pmod 1$. In $T^3$ these glue into a connected polyhedral surface separating the “inner” and “outer” labyrinths. Each extra unit of period in the $x$-direction adds one more handle (successive copies along $x$ glue by a tube), increasing the genus by $1$. Thus the resulting boundary is connected and has genus $g$.
