...
* **($\Leftarrow$)** Conversely, suppose $X$ is compact Hausdorff. Since $p$ is a local homeomorphism, it is also a proper map, and thus $\tilde{X}$ is compact as the continuous image of a compact space. To show that $\tilde{X}$ is Hausdorff, let $\tilde{x}_1, \tilde{x}_2 \in \tilde{X}$ be distinct points. Since $X$ is Hausdorff, their images $p(\tilde{x}_1)$ and $p(\tilde{x}_2)$ are separated by disjoint open neighborhoods in $X$. The preimages of these neighborhoods under $p$ will be disjoint open sets in $\tilde{X}$, showing that $\tilde{X}$ is Hausdorff.
