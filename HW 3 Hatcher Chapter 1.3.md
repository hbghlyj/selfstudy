...
The covering map takes each sphere onto that in $X$, and the line segments onto the components of $S^1$ with orientation. This is a covering space in the same way that the previous was. Moreover, it is simply connected since we can contract each segment, and once more get an infinite wedge of spheres. 
