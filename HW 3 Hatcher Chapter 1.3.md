...
To show $\tilde{X}$ is compact, let $`\{U_\alpha\}_{\alpha \in A}`$ be an open cover of $\tilde{X}$. For each $x \in X$, choose an evenly covered neighborhood $V_x$ such that $`p^{-1}(V_x) = \bigsqcup_{i=1}^{n_x} W_{x,i}`$ where each $W_{x,i}$ is open in $\tilde{X}$ and $p$ maps $W_{x,i}$ homeomorphically onto $V_x$. The fiber $`p^{-1}(x) = \{\tilde{x}_{x,1}, \ldots, \tilde{x}_{x,k_x}\}`$ is finite. For each $`\tilde{x}_{x,j} \in p^{-1}(x)`$, choose some $`U_{\alpha(x,j)}`$ from the cover containing $\tilde{x}_{x,j}$. Let $W_{x,j}$ be the sheet containing $\tilde{x}_{x,j}$. The set $`U_{\alpha(x,j)} \cap W_{x,j}`$ is open in $W_{x,j}$, and since $p: W_{x,j} \to V_x$ is a homeomorphism, $`p(U_{\alpha(x,j)} \cap W_{x,j})`$ is open in $V_x$. Define $`V'_x = V_x \cap \bigcap_{j=1}^{k_x} p(U_{\alpha(x,j)} \cap W_{x,j})`$, which is an open neighborhood of $x$ in $X$ (a finite intersection of open sets). 

The collection $`\{V'_x \mid x \in X\}`$ is an open cover of $X$. Since $X$ is compact, there exists a finite subcover $`\{V'_{x_1}, \ldots, V'_{x_m}\}`$. For each $i \in \{1, \ldots, m\}$, the preimage $`p^{-1}(V'_{x_i})`$ is covered by the finitely many sets $`\{U_{\alpha(x_i,j)} \mid j = 1, \ldots, k_{x_i}\}`$. This is because for any $`\tilde{y} \in p^{-1}(V'_{x_i})`$, we have $`p(\tilde{y}) \in V'_{x_i} \subseteq V_{x_i}`$, so $\tilde{y}$ lies in some sheet $W_{x_i,j}$. By the definition of $V'_{x_i}$, we have $`p(\tilde{y}) \in p(U_{\alpha(x_i,j)} \cap W_{x_i,j})`$, which means $`\tilde{y} \in U_{\alpha(x_i,j)} \cap W_{x_i,j} \subseteq U_{\alpha(x_i,j)}`$. Therefore, the finite collection $`\{U_{\alpha(x_i,j)} \mid i = 1, \ldots, m, j = 1, \ldots, k_{x_i}\}`$ is a finite subcover of $\tilde{X}$. Thus $\tilde{X}$ is compact.
