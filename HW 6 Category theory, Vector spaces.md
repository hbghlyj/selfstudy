...
(⇐) Now suppose the $a_i$'s are all distinct. Since $V$ is the space of polynomials of degree at most $n$, any polynomial is uniquely determined by its values at $n+1$ distinct points, as the difference of any two such polynomials is a polynomial of degree at most $n$ with $n+1$ roots, and thus must be zero. Thus, for any vector in $\mathbb{R}^{n+1}$, there exists a unique polynomial in $V$ that maps to it under $\phi$. This shows that $\phi$ is invertible.
